...
def swine_align(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    GCD = 1
    for i in range(min(player_score,opponent_score)):
        if player_score%(i+1) == 0 and opponent_score%(i+1) ==0:
            GCD = i
    if GCD >= 10 : return True
    else : return False
    # END PROBLEM 4a
...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    if player_score - opponent_score < 0 and player_score - opponent_score > -3 : return True
    else : return False
    # END PROBLEM 4b
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def h(s0, s1):
        high = running_high
        if who == 1:
            now_score = s1 - last_score
            if now_score > running_high:
                print(now_score,"point(s)! The most yet for Player",who)
                high = now_score
            return announce_highest(who, s1, high)
        else :
            now_score = s0 - last_score
            if now_score > high:
                print(now_score,"point(s)! The most yet for Player",who)
                high = now_score
            return announce_highest(who, s0, high)
    return h 
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_ch = 1
    best_score = 0
    dice_method = make_averaged(roll_dice, trials_count)
    for i in range(10):
        score = dice_method(i+1, dice)
        if score > best_score:
            best_score = score
            best_ch = i+1
        elif score == best_score and i+1 < best_ch:
            best_ch = i+1
        else: continue
    return best_ch
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if bacon_score >= cutoff or extra_turn(score + bacon_score, opponent_score) == True:
        return 0
    else: return num_rolls
    # END PROBLEM 11
